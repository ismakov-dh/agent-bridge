...
from queue_fixture import install_queue, messages
...
class NativeClaudeTest(unittest.TestCase):
    def test_native_idle_subscription_and_notice_with_mock_model(self):
        self.exercise_native_sender(queue_failure=False)

    def test_native_sender_sees_queue_failure(self):
        self.exercise_native_sender(queue_failure=True)

    def exercise_native_sender(self, queue_failure):
        with tempfile.TemporaryDirectory(prefix="xsm-native-idle-", dir="/tmp") as temp:
                ...
                def do_POST(self):
                    ...
                    if counter == 1:
                        block = {"type": "tool_use", "id": "toolu_idle_fixture", "name": "SendMessage",
                                 "input": {"to": "codex-idle-fixture", **(
                                     {"message": "Fixture: report queue failure to me."} if queue_failure
                                     else {"notify_when_idle": True})}}
                        reason = "tool_use"
                    else:
            ...
            env.update(CLAUDE_CONFIG_DIR=str(root / "claude"), XSM_DATA_DIR=str(root / "data"),
                       XSM_CODEX_BIN=install_queue(root), ANTHROPIC_API_KEY="fixture-only",
            ...
            with patch.dict(os.environ, env, clear=True):
                ...
                with (root / "out.log").open("w") as out, (root / "err.log").open("w") as err:
                    ...
                    try:
                        ...
                        if queue_failure:
                            notice_turn = after_tool
                            if 'dropped' not in json.dumps(notice_turn).lower():
                                notice_turn = calls.get(timeout=20)
                            self.assertIn('dropped', json.dumps(notice_turn).lower())
                        else:
